...
server <- function(input, output, session) {
  ...
  ####################
  {
    ...
    invalidate<-reactiveValues(pw = 0)
    ...
    observeEvent(input$admin_submitNewPassword, {
      ...
      if(input$admin_newPassword!=input$admin_newPasswordValidation){
        ...
        if(input$admin_newPassword %>% nchar()<8 | 
          ...
          removeModal()
          invalidate$pw<-invalidate$pw+1
          updateTextInput(session, "adminPass", value = input$admin_newPassword)
        }
      }
    })
    ...
    output$adminPage<-renderUI({
      ...
      if(inputPass()==adminPass()){
        ...
        #if(isTruthy(input$adminPass)){
        if(isTruthy(input$admin_newPassword)){
          if(input$admin_newPassword %>% paste0("") %>% sha256() %>% as.character()!= adminPass()){
